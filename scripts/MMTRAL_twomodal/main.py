...
def test(model, target_test_loader):
    ...
    with torch.no_grad():
        for img_data, input_ids, mask, target in target_test_loader:
            img_data, input_ids, mask, target = img_data.to(DEVICE), input_ids.to(DEVICE), mask.to(DEVICE),target.to(DEVICE)
            feature_weights, s_output = model.predict(img_data, input_ids, mask)
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer,scheduler):
    ...
    for e in tqdm(range(args.n_epoch)):
        ...
        for _ in tqdm(range(n_batch)):
            source_img,source_input_ids,source_mask,label_source = next(iter_source)
            target_img,target_input_ids,target_mask,_ = next(iter_target)
            source_img,source_input_ids,source_mask,label_source = source_img.to(DEVICE),source_input_ids.to(DEVICE),source_mask.to(DEVICE),label_source.to(DEVICE)
            target_img,target_input_ids,target_mask = target_img.to(DEVICE),target_input_ids.to(DEVICE),target_mask.to(DEVICE)
            optimizer.zero_grad()
            label_source_pred, transfer_loss = model(source_img,source_input_ids,source_mask,target_img,target_input_ids,target_mask,label_source)
            ...
            try:
                ...
                train_loss_transfer.update(transfer_loss)
