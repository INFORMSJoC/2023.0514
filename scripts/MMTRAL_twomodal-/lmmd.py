...
class LMMDLoss(mmd.MMDLoss):
    def __init__(self, num_class, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, gamma=1.0, max_iter=1000, **kwargs):
        ...
        super(LMMDLoss, self).__init__(
            kernel_type, kernel_mul, kernel_num, **kwargs)
    ...
    def forward(self, source, target, source_label, target_logits):
        if self.kernel_type == 'linear':
            ...
            loss += torch.sum(weight_ss * SS + weight_tt *
                              TT - 2 * weight_st * ST)
            return loss
    ...
    def cal_weight(self, source_label, target_logits):
        ...
        source_label_sum = np.sum(
            source_label_onehot, axis=0).reshape(1, self.num_class)
        ...
        target_logits_sum = np.sum(
            target_logits, axis=0).reshape(1, self.num_class)
        ...
        for i in range(self.num_class):  # (B, C)
            if i in set_s and i in set_t:
                s_tvec = source_label_onehot[:, i].reshape(
                    batch_size, -1)  # (B, 1)
